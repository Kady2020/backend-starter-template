...
> Проверяет, что brew доступен в PATH, и запускает диагностику (doctor).
...
> Показывает, где лежит .app и как он связан с Caskroom (симлинки/папки).
...
> Обновляет Oh My Zsh (темы/плагины, если подключены).
...
> Проверяет путь к python3/pip3 и выводит версии.
